	...
	<div class="row">
		<div class="col-sm-12">
			<form class="form-horizontal" action="admin.php?c=<?php echo $_GET['c']; ?>" method="POST" enctype="multipart/form-data" autocomplete="off">
				...
				<?php $blogimgs = array_diff(scandir('uploads/blog/'.$_GET['id']), array('.','..')); ?>
						...
						<?php for($bim=2;$bim<count($blogimgs)+2;$bim++) { ?>
							<div id="blogimgdel-<?php echo $bim; ?>" class="col-sm-3">
								<a href="#" data-toggle="modal" data-target="#blogimage-<?php echo $bim; ?>">
									<img class="img-responsive" src="uploads/blog/<?php echo $_GET['id']; ?>/<?php echo $blogimgs[$bim]; ?>" style="height:95px;">
								</a>
								<i id="<?php echo $bim;?>" path="../uploads/blog/<?php echo $_GET['id']; ?>/<?php echo $blogimgs[$bim]; ?>" style="color:red;" class="blogimgdel glyphicon glyphicon-remove-circle"></i>
							</div>
							...
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<div class="modal-body">
										<img src="uploads/blog/<?php echo $_GET['id']; ?>/<?php echo $blogimgs[$bim]; ?>" class="img-responsive">
									</div>
								</div>
							</div>
							</div>
...
<?php
$data = getAllDataFromTable($select,'blog',$where,$limit);
if(!empty($data))
{
?>
			...
			<table class="table table-bordered table-striped" style="text-align: center;">
				...
				<?php	for($i=0;$i<sizeof($data);$i++)	{	?>
					<tr id="tr-<?php echo $data[$i]['id'];?>">
						...
						<td>
				<?php $blogimgs = array_diff(scandir('uploads/blog/'.$data[$i]['id']), array('.','..')); ?>
				<?php if(!empty($blogimgs)) { ?>
					<div class="form-group">
						<?php for($bim=2;$bim<count($blogimgs)+2;$bim++) { ?>
							<div class="col-md-3">
								<a href="#" data-toggle="modal" data-target="#blogimage-<?php echo $bim; ?>">
									<img class="img-responsive" src="uploads/blog/<?php echo $data[$i]['id']; ?>/<?php echo $blogimgs[$bim]; ?>">
								</a>
							</div>
							<div id="blogimage-<?php echo $bim; ?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<div class="modal-body">
										<img src="uploads/blog/<?php echo $data[$i]['id']; ?>/<?php echo $blogimgs[$bim]; ?>" class="img-responsive">
									</div>
								</div>
							</div>
							</div>
							...
							<div id="del-<?php echo $data[$i]['id'];?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
								<div class="modal-dialog modal-sm">
									<div class="modal-content">
										...
										<div class="modal-body">
										<center>
											<button class="btn btn-danger" onclick="mydelete('<?php echo $data[$i]['id'];?>','courseid','courses','orders')" data-dismiss="modal"><?php language('agree'); ?></button>
